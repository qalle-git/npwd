...
onNetPromise<void, FormattedProfile[]>(
...
onNetPromise<void, void>("phone:openMatchApp", (reqObj, resp) => {
  MatchService.handleOpenApp(reqObj, resp).catch((e) => {
    matchLogger.error(
      `Error occurred in open app event (${reqObj.source}), Error: ${e.message}`
    );
    resp({ status: "error", errorMsg: "INTERNAL_ERROR" });
  });
});
...
onNetPromise<{ page: number }, FormattedMatch[]>(
...
onNetPromise<Like, boolean>(MatchEvents.SAVE_LIKES, (reqObj, resp) => {
  MatchService.handleSaveLikes(reqObj, resp).catch((e) => {
    matchLogger.error(
      `Error occurred in save likes event (${reqObj.source}), Error: ${e.message}`
    );
    resp({ status: "error", errorMsg: "INTERNAL_ERROR" });
  });
});

onNetPromise<Profile, FormattedProfile>(
...
onNet(MatchEvents.INITIALIZE, () => {
  ...
  MatchService.handleInitialize(_source).catch((e) =>
    matchLogger.error(
      `Error occurred in initialize event (${_source}), Error: ${e.message}`
    )
  );
});

if (
  !config.match.allowEditableProfileName &&
  !config.match.generateProfileNameFromUsers
) {
  const warning =
